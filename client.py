...
class ClientSender(threading.Thread, metaclass=ClientVerifier):
    ...
    def class_verifyer(self):
        if self.sock.type.name == 'SOCK_STREAM':
            self.is_check_sock_stream = self.sock.type.name
        if self.sock.type.name == 'AF_INET':
            self.is_check_if_inet = self.sock.family.name
